...
# Disable WiFi power saving to prevent AP dropouts with connected clients
...
echo "WiFi power saving disabled (prevents AP dropouts)"

echo ""
echo "Step 3.5: Pinning network interface names..."
# Ensures eth0 (onboard LAN) and eth1 (USB Ethernet/WLED link) are always
# correctly named regardless of boot enumeration order.
# PixelPi architecture: eth0 = LAN, eth1 = single USB adapter to unmanaged switch.
# Connect all WLED controllers via an unmanaged switch on eth1 rather than
# using multiple USB Ethernet adapters.

if [ -f "/sys/class/net/eth0/address" ] && [ -f "/sys/class/net/eth1/address" ]; then
    ETH0_MAC=$(cat /sys/class/net/eth0/address)
    ETH1_MAC=$(cat /sys/class/net/eth1/address)

    cat > /etc/udev/rules.d/10-network-names.rules <<EOF
# Pin network interface names by MAC address
# Generated by PixelPi installer - do not edit manually
# eth0 = onboard LAN, eth1 = USB Ethernet adapter (WLED controller network)
SUBSYSTEM=="net", ACTION=="add", ATTR{address}=="$ETH0_MAC", NAME="eth0"
SUBSYSTEM=="net", ACTION=="add", ATTR{address}=="$ETH1_MAC", NAME="eth1"
EOF

    udevadm control --reload-rules
    echo "Interface names pinned: eth0 ($ETH0_MAC), eth1 ($ETH1_MAC)"
else
    echo "WARNING: eth0 and/or eth1 not found - skipping interface name pinning."
    echo "  Ensure the USB Ethernet adapter is plugged in before running the installer,"
    echo "  or run pin-interfaces.sh manually after installation."
fi
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo "You can install it later by re-running the installer"
fi
