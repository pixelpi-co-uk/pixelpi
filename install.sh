...
# Ensure system dnsmasq ONLY serves USB Ethernet adapters (eth1, eth2 etc)
# and never interferes with:
#   - eth0 (built-in Ethernet - MUST always get DHCP from the real network)
#   - wlan0 (WiFi AP - NetworkManager runs its own dnsmasq for this)
# bind-dynamic: Only bind to configured interfaces, not 0.0.0.0
#   Handles USB adapters being plugged/unplugged dynamically
# except-interface: Explicitly exclude eth0 and wlan0 as extra safety
...
sed -i '/^except-interface=eth0$/d' /etc/dnsmasq.conf
echo "bind-dynamic" >> /etc/dnsmasq.conf
echo "except-interface=eth0" >> /etc/dnsmasq.conf
echo "except-interface=wlan0" >> /etc/dnsmasq.conf
systemctl restart dnsmasq 2>/dev/null || true
echo "dnsmasq: configured for USB adapters only (eth0 and wlan0 protected)"
