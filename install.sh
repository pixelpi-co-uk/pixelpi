...
curl -sL https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.0/font/fonts/bootstrap-icons.woff2 -o "$INSTALL_DIR/web/static/fonts/bootstrap-icons.woff2" 2>/dev/null
curl -sL https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.0/font/fonts/bootstrap-icons.woff -o "$INSTALL_DIR/web/static/fonts/bootstrap-icons.woff" 2>/dev/null
...
echo ""
echo "Step 6.5: Configuring NetworkManager to not manage wlan0..."
# Prevent NetworkManager from managing wlan0 (needed for WiFi AP)
mkdir -p /etc/NetworkManager/conf.d
cat > /etc/NetworkManager/conf.d/unmanaged-wlan0.conf << 'EOF'
[keyfile]
unmanaged-devices=interface-name:wlan0
EOF
echo "  NetworkManager configured to leave wlan0 unmanaged"
...
cp systemd/wifi-ap-startup.timer /etc/systemd/system/
cp systemd/wifi-ap-startup.service /etc/systemd/system/
...
echo "WiFi AP timer installed (starts 120 seconds after boot when enabled)"
