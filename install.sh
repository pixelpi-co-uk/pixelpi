...
if ! grep -q "except-interface=wlan0" /etc/dnsmasq.conf 2>/dev/null; then
    echo "" >> /etc/dnsmasq.conf
    echo "# Keep system dnsmasq off wlan0 - NetworkManager handles WiFi AP DHCP" >> /etc/dnsmasq.conf
