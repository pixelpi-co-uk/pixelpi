...
# Ensure system dnsmasq doesn't conflict with NetworkManager's WiFi AP dnsmasq
# Two things are needed:
#   1. bind-dynamic: Only bind to configured interfaces (eth1, eth2 etc), not 0.0.0.0
#      This frees port 67 (DHCP) on wlan0. Also handles USB adapters being plugged/unplugged.
#   2. except-interface=wlan0: Extra safety to explicitly exclude wlan0 from DNS
# Without bind-dynamic, dnsmasq binds DHCP to all interfaces even with except-interface set
sed -i '/^bind-dynamic$/d' /etc/dnsmasq.conf
...
echo "dnsmasq: configured for USB adapters only (wlan0 reserved for WiFi AP)"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Mask hostapd - NetworkManager handles AP mode via wpa_supplicant internally
    # hostapd must NOT run independently or it fights NM for control of the radio
    ...
    echo "You can install it later by re-running this installer"
fi
