...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Clean up old delayed-start service from previous versions
    systemctl stop wifi-ap-delayed-start.service 2>/dev/null
    systemctl disable wifi-ap-delayed-start.service 2>/dev/null
    rm -f /etc/systemd/system/wifi-ap-delayed-start.service
    
    # Ensure NM autoconnect is off if connection already exists
    # (the pixelpi-wifi-ap.service handles boot activation safely)
    nmcli connection modify pixelpi-ap autoconnect no 2>/dev/null
