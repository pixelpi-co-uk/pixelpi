...
"""
            ...
            # Ensure NetworkManager never manages wlan0 (permanent setting)
            nm_conf_dir = '/etc/NetworkManager/conf.d'
            nm_conf_file = f'{nm_conf_dir}/unmanaged-wlan0.conf'
            os.makedirs(nm_conf_dir, exist_ok=True)
            with open(nm_conf_file, 'w') as f:
                f.write('[keyfile]\n')
                f.write('unmanaged-devices=interface-name:wlan0\n')
            logger.info("Configured NetworkManager to permanently ignore wlan0")
            
            # Restart NetworkManager to apply change
            subprocess.run(
    ...
    def enable(self) -> bool:
        """Enable WiFi AP (start on boot via timer)"""
        try:
            ...
            # Ensure IP is assigned NOW for immediate use
            self._assign_ip()
            ...
            # Start hostapd and dnsmasq NOW (but don't enable for direct boot)
            subprocess.run(
            ...
            # Enable timer for automatic startup after boot (120 seconds delay)
            success, _, _ = self._run_command(['/usr/bin/systemctl', 'enable', 'wifi-ap-startup.timer'])
            
            if success:
                logger.info("WiFi AP enabled - will auto-start 120 seconds after boot")
                return True
            else:
                logger.error("Failed to enable WiFi AP startup timer")
                return False
    ...
    def disable(self) -> bool:
        ...
        try:
            ...
            success, _, _ = self._run_command(['/usr/bin/systemctl', 'disable', 'wifi-ap-startup.timer'])
            subprocess.run(
                ['/usr/bin/systemctl', 'stop', 'wifi-ap-startup.timer'],
                capture_output=True
            )
