...
"""
    ...
    def _cleanup_wlan0_dnsmasq(self):
        """Remove any old wlan0 dnsmasq config from previous versions.
        
        NM shared mode handles DHCP for WiFi clients, so dnsmasq should
        only be configured for USB Ethernet interfaces.
        """
        ...
        try:
            ...
            # Check if wlan0 config exists in dnsmasq
            if f'interface={self.interface}' not in config:
                return
            
            # Remove wlan0-specific lines
            lines = config.split('\n')
            cleaned_lines = []
            skip_section = False
            ...
            if new_config != config:
                ...
                logger.info("Removed old wlan0 config from dnsmasq.conf")
                
        except Exception as e:
            logger.warning(f"Error cleaning up dnsmasq config: {e}")
