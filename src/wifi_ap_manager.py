...
"""
    ...
    def _cleanup_wlan0_dnsmasq(self):
        """Ensure system dnsmasq stays off wlan0.
        
        NetworkManager runs its own dnsmasq for WiFi AP shared mode,
        so the system dnsmasq must not bind to wlan0 or there will be
        port conflicts (Address already in use).
        
        This method:
        1. Adds except-interface=wlan0 if missing
        2. Removes any old interface=wlan0 config from previous versions
        """
        ...
        try:
            ...
            # 1. Ensure except-interface=wlan0 is present
            if 'except-interface=wlan0' not in config:
                config += "\n# Keep system dnsmasq off wlan0 - NetworkManager handles WiFi AP DHCP\n"
                config += "except-interface=wlan0\n"
                changed = True
                logger.info("Added except-interface=wlan0 to dnsmasq.conf")
            
            # 2. Remove any old interface=wlan0 config from previous versions
            if f'interface={self.interface}' in config:
                ...
                for line in lines:
                    # Detect start of wlan0 section
                    if 'WiFi Access Point' in line or f'interface={self.interface}' in line:
                        skip_section = True
                        continue
                    
                    # Skip lines that reference wlan0
                    if skip_section and (
                        f'={self.interface}' in line or
                        line.strip() == 'bind-interfaces'
                    ):
                        continue
                    
                    # End of wlan0 section - stop skipping
                    if skip_section and line.strip() and not line.startswith('#'):
                        if f'{self.interface}' not in line:
                            skip_section = False
                ...
                config = '\n'.join(cleaned_lines)
                changed = True
                logger.info("Removed old wlan0 interface config from dnsmasq.conf")
