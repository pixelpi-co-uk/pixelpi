    ...
    def _configure_dnsmasq(self, ip_address: str) -> bool:
        ...
        try:
            ...
            if f'interface={self.interface}' in config:
                # Write back config with port=0 if it was added
                if 'port=0' in config and not os.path.exists(dnsmasq_conf) or 'port=0' not in open(dnsmasq_conf).read():
