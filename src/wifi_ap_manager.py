...
class WiFiAPManager:
    ...
    def __init__(self):
        ...
        self._status_cache = {}
        self._cache_timeout = 30  # seconds - status doesn't change often
    
    def _safe_run(self, cmd: List[str], timeout: int = 10) -> subprocess.CompletedProcess:
        """Run subprocess with timeout - safe for polling. Returns CompletedProcess."""
        try:
    ...
    def _run_command(self, cmd: List[str], timeout: int = 15) -> tuple:
    ...
    def get_config(self) -> Optional[Dict]:
        ...
        def _fetch_config():
            ...
            try:
                # Single nmcli call to get all fields at once
                result = self._safe_run([
                    '/usr/bin/nmcli', '-t', '-f',
                    '802-11-wireless.ssid,802-11-wireless.channel,ipv4.addresses',
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().split('\n'):
                        ...
                        if 'ssid' in key and val:
                            config['ssid'] = val
                        elif 'channel' in key and val:
                            try:
                                config['channel'] = int(val)
                            except ValueError:
                                pass
                        elif 'addresses' in key and val:
                            if '/' in val:
                                config['ip_address'] = val.split('/')[0]
        ...
        return self._get_cached('get_config', _fetch_config)
...
"""
    ...
    def _cleanup_wlan0_dnsmasq(self):
        """Ensure system dnsmasq ONLY serves USB Ethernet adapters.
        
        System dnsmasq must never interfere with:
          - eth0: Built-in Ethernet MUST always get DHCP from the real network
          - wlan0: NetworkManager runs its own dnsmasq for WiFi AP shared mode
        
        Three directives are needed:
          - bind-dynamic: Only bind to configured interfaces, not 0.0.0.0.
            Handles USB adapters appearing/disappearing dynamically.
          - except-interface=eth0: Protect built-in Ethernet from interference.
          - except-interface=wlan0: Protect WiFi AP from port conflicts.
        ...
        try:
            ...
            has_except_eth0 = False
            has_except_wlan0 = False
            ...
            for line in config.split('\n'):
                ...
                if stripped == 'except-interface=eth0':
                    has_except_eth0 = True
                if stripped == 'except-interface=wlan0':
                    has_except_wlan0 = True
                if stripped == f'interface={self.interface}':
                    has_old_interface = True
            
            restart_needed = False
            
            # Ensure bind-dynamic is present
            ...
            # Ensure except-interface=eth0 is present (protect built-in Ethernet)
            if not has_except_eth0:
                subprocess.run(
                    ['sed', '-i', '/^except-interface=eth0$/d', dnsmasq_conf],
                    capture_output=True
                )
                with open(dnsmasq_conf, 'a') as f:
                    f.write('except-interface=eth0\n')
                restart_needed = True
                logger.info("Added except-interface=eth0 to dnsmasq.conf")
            
            # Ensure except-interface=wlan0 is present (protect WiFi AP)
            if not has_except_wlan0:
                subprocess.run(
