...
# File lock to prevent concurrent dnsmasq.conf modifications
_config_lock_path = '/tmp/pixelpi-dnsmasq.lock'
...
class DnsmasqConfig:
    ...
    def _acquire_lock(self):
        """Acquire file lock to prevent concurrent config modifications"""
        self._lock_fd = open(_config_lock_path, 'w')
    ...
    def _run_command(self, cmd: List[str], timeout: int = 15) -> bool:
    ...
    def restart_service(self) -> bool:
        """Restart dnsmasq service and bounce WiFi AP if active.
        
        When system dnsmasq restarts, it can disrupt NetworkManager's
        internal dnsmasq instance that provides DHCP for the WiFi AP.
        Bouncing the AP forces NM to re-establish its dnsmasq cleanly.
        """
        logger.info("Restarting dnsmasq service...")
        result = self._run_command(['/usr/bin/sudo', '/usr/bin/systemctl', 'restart', 'dnsmasq'])
        
        if result:
            self._bounce_wifi_ap()
        
        return result
    
    def _bounce_wifi_ap(self):
        """Bounce WiFi AP if active to restore its DHCP after dnsmasq restart"""
        try:
            import time
            ...
            if 'pixelpi-ap' in (ap_check.stdout or ''):
                logger.info("WiFi AP active - bouncing to restore DHCP")
                subprocess.run(
            ...
            logger.warning(f"Error bouncing WiFi AP: {e}")
