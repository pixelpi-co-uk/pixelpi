...
class AdapterManager:
    ...
    def _configure_dhcp(self, interface: str, ip_address: str, subnet_mask: str) -> bool:
        ...
        try:
            ...
            # If this is the first interface being configured, add bind-dynamic
            # bind-dynamic makes dnsmasq only bind to configured interfaces
            # (not wildcard 0.0.0.0) and handles interfaces appearing/disappearing
            # Note: bind-dynamic replaces the older bind-interfaces directive
            if 'bind-dynamic' not in config and 'bind-interfaces' not in config:
                dhcp_config += "bind-dynamic\n"
            
            # Ensure eth0 and wlan0 are always excluded from dnsmasq
            # eth0 MUST always get DHCP from the real network (remote access)
            # wlan0 is managed by NetworkManager for WiFi AP
            if 'except-interface=eth0' not in config:
                dhcp_config += "except-interface=eth0\n"
            if 'except-interface=wlan0' not in config:
                dhcp_config += "except-interface=wlan0\n"
            ...
            if restart_result.returncode == 0:
                ...
                # Bounce WiFi AP if it was active - dnsmasq restart can disrupt
                # NetworkManager's internal dnsmasq for the AP's DHCP
                ap_check = subprocess.run(
                    ['/usr/bin/nmcli', '-t', '-f', 'NAME,TYPE', 'connection', 'show', '--active'],
                    capture_output=True, text=True
                )
                if 'pixelpi-ap' in (ap_check.stdout or ''):
                    logger.info("WiFi AP is active - bouncing to restore DHCP")
                    subprocess.run(
                        ['/usr/bin/sudo', '/usr/bin/nmcli', 'connection', 'down', 'pixelpi-ap'],
                        capture_output=True
                    )
                    time.sleep(2)
                    subprocess.run(
                        ['/usr/bin/sudo', '/usr/bin/nmcli', 'connection', 'up', 'pixelpi-ap'],
                        capture_output=True
                    )
                    logger.info("WiFi AP bounced successfully")
